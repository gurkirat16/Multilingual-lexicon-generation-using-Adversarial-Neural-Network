...
def get_word_translation_accuracy(lang1, word2id1, emb1, lang2, word2id2, emb2, method, dico_eval):
    ...
    dico = load_dictionary(path, word2id1, word2id2).to(emb1.device)
    ...
    for k in [1, 5, 10]:
        ...
        for i, src_id in enumerate(dico[:, 0].cpu().detach().numpy()):
            matching[src_id] = min(matching.get(src_id, 0) + _matching[i], 1)
        # evaluate precision@k
        print(list(matching.values()))
        if isinstance(list(matching.values())[0], int):
            precision_at_k = 100 * np.mean(list(matching.values()))
        else:
            precision_at_k = 100 * np.mean([i.cpu().detach().numpy() for i in list(matching.values())])
